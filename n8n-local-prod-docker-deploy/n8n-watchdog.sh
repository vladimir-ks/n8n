...
if ! docker ps | grep -q n8n-n8n-1; then
  ...
  cd "/Users/vmks/!LEARNprogramming/n8n/n8n-local-prod-docker-deploy" && docker compose up -d
fi
...
if ! docker ps | grep -q n8n-nginx-1; then
  ...
  cd "/Users/vmks/!LEARNprogramming/n8n/n8n-local-prod-docker-deploy" && docker compose up -d nginx
fi
